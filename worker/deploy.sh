...
DB_NAME="paikka-halytin-db"
BUCKET_NAME="paikka-halytin-kuvat"
...
aseta_salaisuus "ADMIN_SALASANA"
...
if [ -n "$WORKER_URL" ]; then
  ...
  echo "  VITE_LOYTO_API_URL=$WORKER_URL"
else
  echo "Deploy valmis. Katso Workerin URL yllä olevasta tulosteesta ja aseta se"
  echo "VITE_LOYTO_API_URL-muuttujaksi app/.env.local:iin ja GitHub Actionsiin."
fi

if [ "$R2_KAYTOSSA" -eq 0 ]; then
  echo
  echo "HUOM: R2-kuvavarasto ohitettiin. Kätkökuvat (GET /kuvat/:tiedostonimi) eivät toimi"
